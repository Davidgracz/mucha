...
class ConsoleBrainUI:
    ...
    def _render(self, snap: dict[str, Any]):
        ...
        ldiag = snap.get("language_diag", {})
        lang.add_row("język", f"{snap['language_tokens']:,} znaków / {snap['language_unique']:,} unikalnych")
        lang.add_row("tryb języka", str(ldiag.get("mode", "characters")))
        lang.add_row("wiadomości", f"{int(ldiag.get('messages', 0)):,}")
        lang.add_row("przejścia znaków", f"{int(ldiag.get('transitions', 0)):,}")
